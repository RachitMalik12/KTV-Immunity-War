...
const size_t PROJECTILE_FIRE_RATE = 500;
...
bool WorldSystem::step(float elapsed_ms_since_last_update) {
	...
	if (player1.isFiringProjectile && next_projectile_fire_player1 < 0.f) {
		next_projectile_fire_player1 = PROJECTILE_FIRE_RATE;
		switch (player1.firingDirection) {
			case 0: // up
				createProjectile(renderer, playerMotion.position, { 0, -1.f * PROJECTILE_SPEED * defaultResolution.scaling }, player_wizard);
				break;
			case 1: // right
				createProjectile(renderer, playerMotion.position, { PROJECTILE_SPEED * defaultResolution.scaling, 0 }, player_wizard);
				break;
			case 2: // down
				createProjectile(renderer, playerMotion.position, { 0, PROJECTILE_SPEED * defaultResolution.scaling }, player_wizard);
				break;
			case 3: // left
				createProjectile(renderer, playerMotion.position, { -1.f * PROJECTILE_SPEED * defaultResolution.scaling, 0 }, player_wizard);
				break;
		}
	}
	...
	if (registry.players.entities.size() > 1) {
		...
		if (player2.isFiringProjectile && next_projectile_fire_player2 < 0.f) {
			next_projectile_fire_player2 = PROJECTILE_FIRE_RATE;
			double x, y;
